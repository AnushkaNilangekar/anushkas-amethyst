...
const LINKS = [
  { label: 'Email',    icon: '✉',           getHref: (p) => `mailto:${p.email}`, getText: (p) => p.email,                            target: '_self'  },
  { label: 'LinkedIn', faIcon: faLinkedinIn, getHref: (p) => p.linkedin,          getText: () => 'linkedin.com/in/anushka-nilangekar', target: '_blank' },
  { label: 'GitHub',   faIcon: faGithub,     getHref: (p) => p.github,            getText: () => 'github.com/AnushkaNilangekar',       target: '_blank' },
  { label: 'Resume',   icon: '↗',            getHref: (p) => p.resume,            getText: () => 'View resume',                        target: '_blank' },
];
    ...
    <section id="contact" className="relative">
      ...
      <div className="section">
        ...
        >
          ...
          <h2 className="font-display font-bold text-gradient mb-4" style={{ fontSize: 'clamp(2rem, 5vw, 3.2rem)' }}>
            Let's talk
          </h2>
            ...
            {status === 'sent' ? (
              ...
              <>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] tracking-widest uppercase mb-1.5" style={{ color: 'rgba(221,214,254,0.35)' }}>Name</label>
                    <input type="text" className="input-field" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-[10px] tracking-widest uppercase mb-1.5" style={{ color: 'rgba(221,214,254,0.35)' }}>Email</label>
                    <input type="email" className="input-field" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
                  </div>
                </div>

                <div>
                  <label className="block text-[10px] tracking-widest uppercase mb-1.5" style={{ color: 'rgba(221,214,254,0.35)' }}>Message</label>
                  <textarea rows={5} className="input-field resize-none" value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} />
                </div>

                {status === 'error' && (
                  <p className="text-xs" style={{ color: 'rgba(245,158,11,0.7)' }}>Something went wrong — try emailing directly.</p>
                )}
                ...
                <button
                  ...
                  onMouseEnter={(e) => (e.currentTarget.style.boxShadow = '0 0 28px rgba(245,158,11,0.18)')}
